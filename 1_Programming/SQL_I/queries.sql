...
*/
/*
SELECT s.name AS name, t.gpa AS gpa FROM student AS s
JOIN term_gpa as t ON s.id = t.id
WHERE s.name IN ('Edith Warton', 'Henry James') AND t.term = 2011 OR t.term = 2012;
*/
...
*/
/*
SELECT s.name AS name, ROUND(MAX(t.gpa),2) AS max_gpa FROM student AS s
JOIN term_gpa as t ON s.id = t.id
